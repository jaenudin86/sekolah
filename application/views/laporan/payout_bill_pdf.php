...
<body>
  ...
  <div class="tp"> TAHUN PELAJARAN <?php foreach ($period as $row) : ?> <?= ($this->input->get('n') == $row['id']) ? $row['period_start'] . '/' . $row['period_end'] : '' ?><?php endforeach; ?></div>
  ...
  <table width="100%" border="0">
    ...
    <tr>
      ...
      <td>:</td>
      <?php foreach ($siswa as $row) : ?>
        <td><?= $row['class_name'] ?> <?= ($row['majors_name'])? ' - ' . $row['majors_name'] : '' ?></td>
      <?php endforeach; ?>
    </tr>
  </table><br>
  <?php if ($f != NULL) { ?>
  ...
  <?php } else redirect('payout?r=' . $f['r'])  ?>
  <table>
		<tr>
			<td>&nbsp;</td>
		</tr>
		...
		<tr>
			<?php foreach ($siswa as $row) : ?>
				<td colspan="2" style="text-align: left; padding-left: 25px;">( <?= ($user) ? $user['nama'] : '............................................' ?> )</td>
			<?php endforeach ?>
		</tr>
	</table>

			...
			<div class="col-md-4">
				<div class="box box-success">
					...
					<div class="box-footer">
						<a href="<?php echo site_url('payout?' . 'n=' . $payment['period_period_id'] . '&' . 'r=' . $student['student_nis']) ?>" class="btn btn-primary pull-right">Kembali</a>
					</div>
				</div>
			</div>
			<div class="col-md-8">
				<div class="box box-success">
					<div class="box-header with-border">
						<form action="<?php echo site_url('payout/multiple'); ?>" method="post">
						...
						<table class="table table-striped table-hover">
							...
							<tbody>
								<tr ng-repeat="item in payouts" ng-style="item.bulan_status == 1 ? {'color' : '#00E640'} : {'color' : 'red'}">
									...
									<td>
										<a class="btn btn-success btn-xs" ng-class="item.bulan_status == 0 ? 'disabled': 'view-pdf'" target="_blank" title="Cetak Slip" href="<?php echo site_url('payout/printPay/') ?>{{item.payment_payment_id}}/{{item.student_student_id}}/{{item.bulan_id}}"><span class="fa fa-print"></span> Cetak</a>
									</td>
								</tr>
							</tbody>
						</table>
...
<script>
	...
	App.controller('payoutCtrl', function($scope, $http) {
		...
		$scope.pay = function(bulan_id) {
			$.ajax({
				method: "Get",
				url: SITEURL + "payout/pay/" + payment_id + '/' + student_id + '/' + bulan_id,
		...
		$scope.notPay = function(bulan_id) {
			$.ajax({
				method: "Get",
				url: SITEURL + "payout/not_pay/" + payment_id + '/' + student_id + '/' + bulan_id,
				success: function(data) {
					$scope.getPayout();
				}
			});
